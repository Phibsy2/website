...
export default async function WalkerDashboard() {
  ...
  // Get group walk stats
  const groupSlotsToday = todaySlots.filter((s) => s.isGroupSlot).length
  const totalGroupSlots = await prisma.walkSlot.count({
    where: {
      walkerId: walker.id,
      isGroupSlot: true,
      date: { gte: today },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Link href="/walker/groups">
          <Card className="hover:border-green-200 hover:shadow-md transition-all cursor-pointer h-full">
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold">{groupSlotsToday}</p>
                  <p className="text-sm text-gray-500">Gruppen heute</p>
                </div>
              </div>
            </CardContent>
          </Card>
